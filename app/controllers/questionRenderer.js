...
var autoComplteQuestion = function(questionObject) {
	...
	if (questionObject.isAQuestion == false)
	...
	if (questionObject.type == "date") {
		...
		questionObject.questionResponse = "<ques:parameterName>" + questionObject.alcrmQuestionID + "</ques:parameterName>" + "<ques:parameterValue>" + questionObject.selections[0].value + "</ques:parameterValue>";
	} else if (questionObject.type == "multiSelect") {
		questionObject.value[0] = questionObject.selections[0].value;
		questionObject.displayValue.value = questionObject.selections[0].displayValue;

		questionObject.questionResponse = "<ques:parameterName>" + questionObject.alcrmQuestionID + "</ques:parameterName>" + "<ques:values>" + questionObject.selections[0].value + "</ques:values>";
	}

	if (questionObject.alcrmQuestionID == "I_COLLECTOR_NAME") {
		questionObject.value[0] = "Dogfish tester";
		questionObject.displayValue.value = questionObject.value[0];
...
var newTestIfMandatory = function(questionObject) {
	...
	if (questionObject.validation.matchAllMandatoryRestrictions == true) {
		...
		for (var conditionalIndex = 0; conditionalIndex < conditionalMandatory.length; conditionalIndex++) {
			...
			if (conditionalMandatory[conditionalIndex].value == null)
				return false;
...
var findQuestionByAlcrmGroupAndName = function(alcrmQuestionID, alcrmGroupType, associatedFileName) {
	...
	for (var sectionIndex = 0; sectionIndex < sectionListLength; sectionIndex++) {
		...
		if (sectionList[sectionIndex].alcrmGroupType == alcrmGroupType) {
			if (searchByAssociatedFileName == true && sectionList[sectionIndex].associatedFileName != associatedFileName)
				continue;
...
exports.goToFirstUnanswered = function() {
	...
	for (var sectionIndex = 0; sectionIndex < sectionList.length; sectionIndex++) {
		...
		if (sectionList[sectionIndex].pageType == "coreQuestion")
			continue;
...
var startCensesTimer = function(question) {
	...
	if (questionRef != null) {
		...
		var questionResponse = "<ques:parameterName>" + questionRef.question.alcrmQuestionID + "</ques:parameterName>" + "<ques:parameterValue>" + timeString + "</ques:parameterValue>" + "<ques:notes>" + questionRef.question.notes + "</ques:notes>";
...
var updateAndReturnQuestion = function(question, value, displayValue) {
	if (question.template == "singleSelectTemplate") {
		...
		var questionResponse = "<ques:parameterName>" + question.alcrmQuestionID + "</ques:parameterName>" + "<ques:parameterValue>" + value[0] + "</ques:parameterValue>";
...
exports.saveCurrentlySelectedQuestion = function() {
	if (questionSelected.question != null) {
		...
		if (questionSelected.question.template == "textFieldTemplate" && Alloy.Globals.currentlyFocusedTF.TextField != null) {
			...
			question.questionResponse = "<ques:parameterName>" + question.alcrmQuestionID + "</ques:parameterName>" + "<ques:parameterValue>" + newValue + "</ques:parameterValue>";
